...
// Features: Rate limiting via Upstash Redis, input sanitization, anti-automation.
// ══════════════════════════════════════════════════════════════
...
function rateLimitResponse(retryAfterMs) {
  ...
  return new Response(JSON.stringify({ error: "تم تجاوز الحد المسموح. يرجى المحاولة بعد دقيقة.", retryAfter }), {
    status: 429,
